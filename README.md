...
| 指令 | 说明 |
|------|------|
| `刮刮乐 [数量]` | 购买 5 万档刮刮卡（默认），支持一次多张（合并转发） |
| `刮刮乐2 [数量]` | 购买 2 万档刮刮卡 |
| `刮刮乐3 [数量]` | 购买 3 万档刮刮卡 |
| `刮刮乐5 [数量]` | 购买 5 万档刮刮卡 |
| `刮全部` | 购买各档位今日基础限购剩余的全部额度（不含追加额度） |
| `刮取钱` | 周期内顺序随机的补助 |
| `刮商店` | 回声书签商店：查看货架/购买方斯、追加额度、护符 |
| `刮余额` | 查看余额和游戏统计数据 |
| `富爪榜` | 查看累计盈亏排行榜（图片） |
| `富爪日榜` | 查看今日盈亏排行榜（图片） |
...
> - `give_money` / `give_cards` 会根据说话者身份自动分流：管理员直接发放；普通用户只能提交申请，实际发放由管理员批准后执行。
> - 可通过 `enable_llm_tools` 配置项一键关闭 LLM 自然语言指令。

## 卡片说明

| 档位 | 售价 | 当日限购 | 指令 | 最高奖金 | 布局 | 单格奖金档位 |
|------|-----|----------|------|---------|-----|--------------|
| **2 万卡** | 20,000 | 20 | /刮刮乐2 [数量] | 50 万 | 3X5 | 1万 · 2万 · 3万 · 5万 · 10万 · 15万 · 20万 · 30万 · 50万 |
| **3 万卡** | 30,000 | 30 | /刮刮乐3 [数量] | 150 万 | 3X5 | 1万 · 2万 · 3万 · 5万 · 10万 · 15万 · 20万 · 30万 · 50万 · 80万 · 100万 · 150万 |
| **5 万卡** | 50,000 | 50 | /刮刮乐 [数量] 或 /刮刮乐5 [数量] | 250 万 | 3X4 | 2万 · 5万 · 10万 · 15万 · 20万 · 30万 · 50万 · 80万 · 100万 · 150万 |

- LLM 工具 `scratch_ntc_card` 支持 `tier` 参数指定档位（20000 / 30000 / 50000，默认 50000）

## 道具系统

**回声书签** 🔖：购卡时自动获得（仅基础限购额度内），可在 **回声书签商店**（`/刮商店`）兑换方斯、追加购卡额度与保底护符。
...
无参数输入 `刮商店` 可查看货架与当前书签数；带参数可直接购买：

| 商品 | 价格 | 用法 | 效果 |
|------|------|------|------|
| 方斯 | 1 书签 = 1,000 方斯 | `刮商店 方斯 [数量]` | 兑换方斯余额 |
| 追加额度(2万) | 10 书签/张 | `刮商店 卡2 [数量]` | 2 万档今日追加购卡额度 |
| 追加额度(3万) | 30 书签/张 | `刮商店 卡3 [数量]` | 3 万档今日追加购卡额度 |
| 追加额度(5万) | 50 书签/张 | `刮商店 卡5 [数量]` | 5 万档今日追加购卡额度 |
| 玛门的大手 | 1,000 书签 | `刮商店 大手` | 今日基础额度购卡**未中奖**全额返还成本 |
| 猫丸的后门 | 1,500 书签 | `刮商店 后门` | 今日基础额度购卡**中奖低于成本**返还差额 |

> 💡 说明：
> - 护符**每日限购一种**（当日有效，次日失效）；大手/后门互斥，同一天只能激活其一。
> - 护符仅对**基础限购额度**内购卡生效，追加额度/管理员发放的额外额度不触发。

## 概率系统

基于异环实际数据提取的概率。先按概率抽取总奖金，再分解到各个格子中。
